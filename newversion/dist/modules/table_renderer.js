...
export function createTableRendererModule(opts = {}) {
  ...
  function schemaFromTemplate(template) {
    ...
    return {
      ...
      // Backward compatibility: older templates might use `type` instead of `dataType`.
      fields: cols.map((c) => {
        const dt = c?.dataType ?? c?.type;
        return { key: c.key, label: c.label, type: fieldType(dt) };
      })
    };
  }
  ...
  function applySortToRows(rows, sort, schema, dataset) {
    ...
    while (i < rows.length) {
      ...
      while (i < rows.length) {
        const r = rows[i];
        // Heuristic: subrows have parentId pointing to owner rowId in engine view.
    ...
    const cmp = async (aGrp, bGrp) => {
      ...
      if (typeof aVal === 'number' && typeof bVal === 'number') {
        ...
        res = String(aVal).localeCompare(String(bVal), 'uk', { sensitivity: 'base' });
      }
...
currentTemplateId = resolved.journal?.templateId ?? null;
          ...
          // Apply A→Я / Я→А sorting (view-only, does not change stored ordering)
          ...
          for (const col of view.columns) {
            ...
            // Sort toggle on header click (ignore resize zone near right edge)
            thTitle.dataset.colKey = col.columnKey;
            thTitle.addEventListener('click', async (ev) => {
              try {
                const r = thTitle.getBoundingClientRect();
                const edgePx = 6;
                if ((r.right - ev.clientX) <= edgePx) return; // resize gesture
              } catch (_) {}
...
// Fixed action columns at far right (Transfer / Context)
{
  ...
  ctxBtn.addEventListener('click', async (ev) => {
    ev.stopPropagation();
      ...
      // Row ids can point to a subrow (e.g. "<rowId>::sub::<n>") depending on the current view.
      // For any row-level operation (delete whole row, add/remove subrows) we must resolve the owner row id.
      const _p0 = parseSubrowId(row.rowId);
      const ownerRowId = _p0 ? _p0.ownerId : row.rowId;

    // New: SWS (SettingsWindow v2) modal for row actions (works better on desktop + mobile/WebView)
    const SW = window.SettingsWindow;
    if (!SW || typeof SW.openCustomRoot !== 'function') {
      // Fallback: keep old modal if SWS not available
      try { window.UI?.toast?.show?.('SWS не завантажено'); } catch {}
      return;
    }
    try { SW.init?.(); } catch (_) {}

    const openDeleteScreen = async () => {
      // Keep "armed" state within the same rendered screen.
      // Previous implementation re-opened the screen which reset the flag,
      // so the button never reached the actual delete branch.
      let deleteArmed = false;
      SW.push({
        title: 'Видалення',
        subtitle: 'Оберіть дію',
        content: (ctx) => {
          const ui = ctx.ui;
          const wrap = ui.el('div', '', '');

          const rowCard = ui.card({
            title: 'Всю строку',
            description: 'Видаляє батьківську строку та всі її підстроки.',
            children: []
          });
          const rowBtn = ui.el('button', `sws-btn sws-btn-danger ${deleteArmed ? 'is-armed' : ''}`, deleteArmed ? 'Підтвердити видалення' : 'Видалити всю строку');
          rowBtn.addEventListener('click', async () => {
            if (!deleteArmed) {
              deleteArmed = true;
              // Update button in-place (avoid re-pushing which would reset state)
              rowBtn.classList.add('is-armed');
              rowBtn.textContent = 'Підтвердити видалення';
              return;
            }
            const rid = ownerRowId;
            // Close modal immediately for UX
            SW.close();
            try {
              // IMPORTANT: delete must update the SAME source that refreshTable() reads.
              // Therefore we persist via saveDataset() (which matches loadDataset()).
              ...
              const nextDataset2 = removeRowCascade(dsNow2, rid);
              await saveDataset(runtime, runtime.storage, currentJournalId, nextDataset2);

              engine.recompute?.(); await requestRefresh();
            } catch (e) {
              console.error('Delete row failed', e);
              try { engine.recompute?.(); await requestRefresh(); } catch {}
            }
          });
          rowCard.appendChild(rowBtn);

          const subCard = ui.card({
            title: 'Підстрочку',
            description: 'Видаляє підстрочку за номером (2..n). №1 — батьківська.',
            children: []
          });
          const subRow = ui.el('div', 'sws-row', '');
          const subInput = ui.el('input', 'sws-input', '');
          subInput.type = 'number';
          subInput.min = '1';
          subInput.placeholder = '№ підстроки';
          subInput.style.maxWidth = '140px';
          const subBtn = ui.el('button', 'sws-btn sws-btn-danger', 'Видалити підстрочку');
          subBtn.addEventListener('click', async () => {
            const n = Number(subInput.value);
            if (!Number.isFinite(n) || n < 1) {
              subInput.focus();
              subInput.classList.add('sws-invalid');
              return;
            }
            // Close modal immediately for UX
            SW.close();
            try {
              // UX model: №1 is the parent row.
              if (n === 1) {
                const dsNowP = await loadDataset(runtime, runtime.storage, currentJournalId);
                const removeRowCascade = (ds, rootId) => {
                  const records = Array.isArray(ds?.records) ? ds.records : [];
                  const byId = new Map(records.map((r) => [String(r.id), r]));
                  const toDelete = new Set();
                ...
                const nextDatasetP = removeRowCascade(dsNowP, ownerRowId);
                await saveDataset(runtime, runtime.storage, currentJournalId, nextDatasetP);
                engine.recompute?.(); await requestRefresh();
                return;
              }
              ...
              // Human numbering: 1 = parent row, 2 = first subrow => index 0
              const subrowId = `${ownerRowId}::sub::${n - 2}`;
              const { dataset: nextDataset2, removed } = engine.removeSubrow(subrowId, dsNow2);
              if (!removed) {
                // We already closed modal; just log and refresh
                console.warn('Subrow not removed', { subrowId });
                engine.recompute?.(); await requestRefresh();
                return;
              }
              await saveDataset(runtime, runtime.storage, currentJournalId, nextDataset2);
              ...
              console.error('Delete subrow failed', e);
              try { engine.recompute?.(); await requestRefresh(); } catch {}
            }
          });
          subRow.append(subInput, subBtn);
          subCard.appendChild(subRow);

          const closeBtn = ui.el('button', 'sws-btn', 'Назад');
          closeBtn.addEventListener('click', () => SW.pop());

          wrap.append(rowCard, subCard, closeBtn);
          return wrap;
        }
      });
    };
    ...
    SW.openCustomRoot(() => {
      SW.push({
        title: 'Дії рядка',
        subtitle: 'Підстроки та видалення',
        content: (ctx) => {
          const ui = ctx.ui;
          const wrap = ui.el('div', '', '');

          // Keep button styling consistent with other actions in this modal
          const addSub = ui.el('button', 'sws-btn sws-btn-primary', 'Додати підстроку');
          addSub.addEventListener('click', async () => {
            ...
            try {
              ...
              const { dataset: nextDataset } = engine.addSubrow(ownerRowId, { cells: initCells }, dsNow);
              await saveDataset(runtime, runtime.storage, currentJournalId, nextDataset);
              engine.recompute?.(); await requestRefresh();
            } catch (e) {
              console.error('Add subrow failed', e);
              try { engine.recompute?.(); await requestRefresh(); } catch {}
            }
          });

          const del = ui.el('button', 'sws-btn sws-btn-danger', 'Видалення…');
          del.addEventListener('click', () => openDeleteScreen());

          const close = ui.el('button', 'sws-btn', 'Закрити');
          close.addEventListener('click', () => SW.close());

          const card = ui.card({ title: 'Дії', description: `Строка: ${row.indexLabel ?? ''}`, children: [addSub, del, close] });
